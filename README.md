...
**Вопрос:** При попытке авторизации Google выдаёт ошибку `403: access_denied` с текстом «Доступ заблокирован: приложение не прошло проверку Google». Что делать?  
**Ответ:** Это означает, что ваше приложение в Google Cloud Console находится в тестовом режиме, а ваш аккаунт не добавлен в список тестовых пользователей. Чтобы исправить это:
1. Зайдите в Google Cloud Console → ваш проект → **APIs & Services** → **OAuth consent screen**.
2. В разделе **Test users** нажмите **ADD USERS** и добавьте свой Gmail-адрес (тот же, который используете для входа).
3. Сохраните изменения и повторите запуск скрипта. Авторизация должна пройти успешно.
